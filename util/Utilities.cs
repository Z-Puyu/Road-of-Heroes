...
namespace Game.util {
    public static class Util {
        ...
        public static Vector2I RandomDirection() {
            return Util.DIRECTIONS[Util.rng.RandiRange(0, Util.DIRECTIONS.Count - 1)];
        }
        ...
        public static (ulong, ulong) InvCantor(ulong n) {
            ulong k = 0;
            while (Util.TriangularNumber(k) <= n) {
                k += 1;
            }
            k -= 1;
            ulong t = Util.TriangularNumber(k);
            ulong x = n - t;
            ulong y = k - x;
            return (x, y);
        }
		...
		public static ulong UniqueId(this Vector2I pair) {
			...
			return Util.CantorPairing(q(pair.X), q(pair.Y));
		}
		...
		public static ulong UniqueId(this (long, long) pair) {
			static ulong q(long z) => (ulong)(z >= 0 ? 2 * z : -2 * z - 1); // Z -> N
			return Util.CantorPairing(q(pair.Item1), q(pair.Item2));
		}
		...
		public static ulong UniqueId(long x, long y) {
			static ulong q(long z) => (ulong)(z >= 0 ? 2 * z : -2 * z - 1); // Z -> N
			return Util.CantorPairing(q(x), q(y));
		}
        ...
        public static int Randi(int inf, int sup) {
            return Util.rng.RandiRange(inf, sup);
        }
        ...
        public static int Randi((int, int) range) {
            return Util.rng.RandiRange(range.Item1, range.Item2);
        }
        ...
        public static double Rand(double inf, double sup) {
            return Util.rng.RandfRange((float)inf, (float)sup);
        }
        ...
        public static bool FairCoin() {
            return Util.rng.RandiRange(0, 1) == 1;
        }
        ...
        public static T RandomSelect<T>(this IList<T> list) {
            return list[Util.Randi(0, list.Count - 1)];
        }
